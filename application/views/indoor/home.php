...
<body>
	<div class="" style="margin: 10px">
		...
		<section class="content">
		  ...
		  <a href="<?= site_url() ?>?group_id=<?= $_GET['group_id']; ?>"><button>Download Data</button></a>


		  <div class="row">
		  	<div class="col-sm-6">
		  		<div class="row justify-content-center">
		  			<h1><b>GRESIK TEK 01</b></h1>
		  			
		  		</div>
		  	</div>
		  	<div class="col-sm-6">
		  		<div class="row justify-content-center">
		  			<h1><b>GRESIK GAS 01</b></h1>
		  		</div>
		  	</div>
		  </div>
					...
					<div class="col-12 col-sm-6 col-md-4 d-flex align-items-stretch card-width" style="margin-bottom: 260px;">
						<div class="card bg-light" height="100%" >                
							...
							<div class="card-header border-bottom-0 text-center text-bold text-dark font-title">
								<a style="font-size: 30px;"><?= $caption[$key]; ?></a>
							</div>
...
//WD
	anychart.onDocumentReady(function () {
		...
		gauge.data([<?php foreach($aqm as $data) {echo $data['wd']; } ?>, <?php foreach($aqm as $data) {echo $data['ws']; } ?>]);//arah mata angin dan arah direction
		...
		gauge.label()
				.text('<span style="color: #64B5F6; font-size: 13px">WD: </span>' +
						'<span style="color: #5AA3DD; font-size: 15px">' + <?php foreach($aqm as $data) {echo $data['wd']; } ?> + '\u00B0' + '</span><br>' +
						'<span style="color: #1976d2; font-size: 13px">WS:</span> ' +
						'<span style="color: #166ABD; font-size: 15px">' + <?php foreach($aqm as $data) {echo $data['ws']; } ?> + 'm/s' + '</span><br>' +
						'<span style="color: #166ABD; font-size: 20px">' + <?php foreach($aqm as $data) {echo $data['temperature']; } ?> + '\u00B0 C' + '</span>').useHtml(true)
		...
		gauge.container('container_<?= $key; ?>');
